...
HCTraceScopeHelper::HCTraceScopeHelper(HCTraceArea const* area, HCTraceLevel level, char const* scope)
    : m_area{ area }, m_level{ level }, m_scope{ scope }, m_id{ GetScopeId() }
{
    HCTraceMessage(m_area, m_level, ">>> %hs (%016llX)", m_scope, m_id);
}

HCTraceScopeHelper::~HCTraceScopeHelper()
{
    HCTraceMessage(m_area, m_level, "<<< %hs (%016llX)", m_scope, m_id);
}
